...
def pob_info_to():
    ...
    headers = {"User-Agent":
                   "Mozilla/5.0 (Linux; Android 5.1.1; C6503) AppleWebKit/537.36 (KHTML, like Gecko)"
    ...
    for num in data_id:
        ...
        for item in all_product:
            ...
            try:
                wizki = item.find_next(class_="gallery-static__images-container").find("svg")  # czy 1 wizka jest obrysowana
                wizki = str(wizki)
                if "svg" in wizki:
        ...
        # df.to_json(f'data/to_d_base_{date}.json')
        print(df)
    end = time.time()
    total = end - start
